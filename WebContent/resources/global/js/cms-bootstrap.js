		...
		function createUploaderIVRText(url) {
			...
			uploaderIVRText.bind('Init', function(up, params) {
				...
				$('#filelist-ivrtext').html("<div>Diese IVR Text Datei hochladen:</div>");
			});
			...
			uploaderIVRText.bind('FileUploaded', function(up, file, response) {
			    ...
			    setTimeout(function () { $('#filelist-ivrtext').html("<div>Diese IVR Text Datei hochladen:</div>"); },2000);
			});
		}
		
		$(function() {
			$("#poidiv").modal({ keyboard: true });
			$("#routediv").modal({ keyboard: true });
			$("#categorydiv").modal({ keyboard: true });
		    $("#poi-list").tablesorter({ sortList: [[1,0]] });
		    $("#route-list").tablesorter({ sortList: [[1,0]] });
		    $("#category-list").tablesorter({ sortList: [[1,0]] });
		    $("#message-list").tablesorter({ sortList: [[1,0]] });
		    $("#language-list").tablesorter({ sortList: [[1,0]] });
